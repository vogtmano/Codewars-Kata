//
//  func isSumOfCubes.swift
//  Katas
//
//  Created by Maks Vogtman on 26/01/2023.
//
...
func isSumOfCubes(_ s: String) -> String {
    ...
    var currentNumber = 0
    var digitsInCurrentNumber = 0
    
    for char in s {
        if let digit = Int(String(char)) {
            currentNumber = currentNumber * 10 + digit
            digitsInCurrentNumber += 1
            if digitsInCurrentNumber == 3 {
                let cubic = pow(Double(currentNumber % 10), 3) + pow(Double((currentNumber / 10) % 10), 3) + pow(Double(currentNumber / 100), 3)
                if cubic == Double(currentNumber) {
                    cubicNumbers.append(currentNumber)
                }
                
                currentNumber = 0
                digitsInCurrentNumber = 0
            }
        } else {
            if digitsInCurrentNumber > 0 {
                let cubic = pow(Double(currentNumber % 10), 3) + pow(Double((currentNumber / 10) % 10), 3) + pow(Double(currentNumber / 100), 3)
                if cubic == Double(currentNumber) {
                    cubicNumbers.append(currentNumber)
                }
    ...
    let sum = cubicNumbers.reduce(0, +)
    return cubicNumbers.map(String.init).joined(separator: " ") + " " + String(sum) + " Lucky"
}
